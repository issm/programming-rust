...
fn main() {
    ...
    if args.len() != 5 {
        ...
        eprintln!("Example: {} mandel1.png 1000x750 -1.20,0.35 -1,0.20", args[0]);
        std::process::exit(1);
    }
...
///
/// `c` がマンデルブロ集合に含まれないなら `Some(i)` を返す．`i` は `c` が原点を中心とする 2 の円から出るまでにかかった繰り返し回数となる
/// `c` がマンデルブロ集合に含まれているらしい場合 (正確にいうと，繰り返し回数の上限に達しても `c` がマンデルブロ集合に含まれないことを示せなかった場合)，`None` を返す．
...
/// <sep> は `separator` 引数で与えられる文字で，<left> と <right> は双方とも `T::from_str` でパースできる文字列
...
fn pixel_to_point (
    ...
    Complex {
        ...
        // ここが引き算となっているのはなぜか?
        // 上に動くと pixel.1 は増えるが，虚部は小さくなるからだ
    }
}
